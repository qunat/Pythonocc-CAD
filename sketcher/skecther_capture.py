...
class sketcher_capture(object):
	...
	def dynamics_point_move_point(self):
		if self.perious_ais_point_ID != None and self.ais_point == None:
			...
			self.parent.Displayshape_core.canva._display.Context.Remove(self.ais_point_dict[self.perious_ais_point_ID],
																		False)
			(x, y, z, vx, vy, vz) = self.parent.Displayshape_core.ProjReferenceAxe()
			# ------------------------------------
			...
			self.parent.Displayshape_core.canva._display.Context.Display(self.ais_point_dict[self.perious_ais_point_ID],
																		 False)
			...
			self.parent.Displayshape_core.canva._display.Context.Redisplay(self.bspline_curve, True,
																		   False)  # 重新计算更新已经显示的物
