...
def read_stl_file(filename):
    ...
    return the_shape
def read_stl_file_threading(filename,self):
    """ opens a stl file, reads the content, and returns a BRep topods_shape object
    """
    if not os.path.isfile(filename):
        raise FileNotFoundError("%s not found." % filename)
    ...
    self.import_shape_succeed=True
    return the_shape
...
def read_iges_file_Thread(filename,self,return_as_shapes=False, verbosity=False, visible_only=False):
    """ read the IGES file and returns a compound
    filename: the file path
    return_as_shapes: optional, False by default. If True returns a list of shapes,
                      else returns a single compound
